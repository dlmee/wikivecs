...
def process_records(db_filename, reverse_mapping, key_list, num_workers, batch_size=32000):
    ...
    with Pool(processes=num_workers) as pool, tqdm(total=total_records, desc="Processing records") as pbar:
        while offset < total_records:
            ...
            # Distribute the batch among the workers
            chunked_batches = [batch[i::num_workers] for i in range(num_workers)]
            results = pool.starmap(process_batch, [(chunk, reverse_mapping, key_list) for chunk in chunked_batches])
            
            # Flatten the list of results
            processed_batch = [item for sublist in results for item in sublist]
...
def main():
    ...
    num_workers = 20  # Set the number of workers here
    ...
    process_records(db_filename, reverse_mapping, key_list, num_workers)
